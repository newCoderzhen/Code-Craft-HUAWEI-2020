...
private:
	...
	const double stepSize = 0.99;
	const int maxIterTimes = 1;
	const double predictTrueThresh = 0.687;
	...
	const int k)
{
    ...
    for (i = k*batch; i < num; i++) {
        ...
        gsV += (sigv-label ) * (dataSet[i].features[index]);
    }
    gsV = gsV /nbatch;   // 归一化的除法
    return gsV;
}
...
void LR::train()
{
    ...
    for (i = 0; i < maxIterTimes; i++) {
        ...
        for(int k=0;k<num_batch;k++){
            ...
            param.wtSet[j] -= stepSize * (gradientSlope(trainDataSet, j, sigmoidVec,k)+lambda* param.wtSet[j]);
            }
        }
       for (int j =num_batch*batch;j<trainDataSet.size();j++) {
        ...
        for (j = 0; j < param.wtSet.size(); j++) {
            param.wtSet[j] -= stepSize * (gradientSlope(trainDataSet, j, sigmoidVec,num_batch)+lambda* param.wtSet[j]);
        }
        if (LOSS&& (i%1==0)){
         loss=lossCal();
         acurracy();
